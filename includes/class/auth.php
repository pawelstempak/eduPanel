<?php
 ...
 $feedback=array();
 unset($LOGGED_IN);
 $hidden_hash_var = $_ENV['HIDDEN_HASH_VAR'];
class auth{
	...
	function user_login($login,$haslo,$login_settings) {
		global $feedback,$data_rej,$l,$cfg,$hidden_hash_var;
		if (!$login || !$haslo) {
			...
			if (!$result || $num < 1){
				...
				$this->user_set_tokens($login,$login_settings);
	...
	//metoda do autoryzacji
	function admin_login($email,$haslo,$login_settings) {
		...
		if ($new_hash && ($new_hash==$hash)) {
			...
			if (!$result || $num < 1) {
				...
				return $feedback;
					...
					//$this->user_set_tokens(mysqli_result($result,0,'login'));//odznaczone umozliwia zalogowanie bezposrednio po aktywacji linka rejestracyjnego
		...
		if ($new_password1 && ($new_password1==$new_password2)) {
			...
			if ($this->account_pwvalid($new_password1)) {
				...
				if ($login && $old_password) {
					...
					if (!$result || $num < 1) {
						$feedback['login_stare_haslo_pl']= 'Niepoprawne stare hasło'.mysqli_error();
						$feedback['login_stare_haslo_en']= 'Incorrect old password'.mysqli_error();
						$feedback['login_stare_haslo_ru']= 'Incorrect old password'.mysqli_error();
						return $feedback;
					} else {
						...
						if (!$result) {
							$feedback['blad_zmiany']= ' Password was not change '.mysqli_error();
...
/*
	...
	*/
	...
	function getUserId() {
			if($_SESSION['login'])
			{
			$q=mysqli_query($this->Sqlobject->db,"SELECT id_user FROM user WHERE login='" . $_SESSION['login'] . "'");
	...
	/*
	function ifSettings() {
			$q=mysqli_query("SELECT * FROM settings WHERE id_user='" . $this->getUserId() . "'");
			$r=mysqli_fetch_array($q);
	...
	function getPromoCode($email) {
				...
				$result=mysqli_fetch_array(mysqli_query($sql));
	...
	function user_register($email,$haslo1) {
		...
		global $feedback,$hidden_hash_var,$data_rej,$cfg;
				...
				if($result_e && $num_e > 0){
					...
					$q = mysqli_query("SELECT * FROM user WHERE email = '".$email."'");
					$r = mysqli_fetch_array($q);

					mysqli_query("UPDATE user SET id_user = '".$r['id_user']."' WHERE id_user = '".$r['id_user']."'");

					if (!$result) {
						$feedback['mysqli']= ' ERROR - '.mysqli_error();
						return $feedback;
					} else {
		...
		if ($login && $haslo1 && $haslo1==$haslo2 && $email && $this->validate_email($email) && $company_name && $phone && $address) {
			...
			if ($this->account_namevalid($login) && $this->account_pwvalid($haslo1)) {
				...
				elseif($result_e && $num_e > 0){
					...
					if (!$result) {
						$feedback['mysql']= ' ERROR - '.mysqli_error();
						return $feedback;
					} else {
						//funkjcja wysylania meila z potwierdzeniem
	...
	function user_getid() {
		...
		if (!$G_USER_RESULT) {
			$G_USER_RESULT=mysqli_query("SELECT * FROM user WHERE user_name='" . $this->user_getname() . "'");
		}
