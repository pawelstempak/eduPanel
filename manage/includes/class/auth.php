...
/*
	...
	*/
	...
	function getAdminId() {
			...
			{
				$q=mysqli_query($this->Sqlobject->db,"SELECT id FROM admin WHERE email='" . $_SESSION['email'] . "'");
	...
	function getUserId($email) {
			$q=mysqli_query($this->Sqlobject->db,"SELECT id_user FROM user WHERE email='" . $email . "'");
			$r=mysqli_fetch_array($q);
			return $r['id_user'];
	}
	/*
	...
	function user_register($name,$lastname,$stanowisko,$udo,$email,$login,$haslo1) {
		...
		global $feedback,$hidden_hash_var,$data_rej,$cfg;
				...
				if($result_e && $num_e > 0){
					...
					$feedback['login_istnieje']=  'Powyższa nazwa użytkownika już istnieje.';
					return $feedback;
				}else{
					...
					$this->Sqlobject->set('stanowisko',$stanowisko);
					$this->Sqlobject->set('udo',$udo);
